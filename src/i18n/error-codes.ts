...
} as const satisfies Record<"en-KE" | "zh-CN", Record<HighRiskErrorCode, string>>;
...
export function translateErrorCode(errorCode: string, locale: "en-KE" | "zh-CN" = "en-KE"): string | null {
  const normalizedCode = errorCode.trim().toUpperCase() as HighRiskErrorCode;
  return errorCodeMessages[locale][normalizedCode] || null;
}
