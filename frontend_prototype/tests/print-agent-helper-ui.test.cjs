...
test("print modal advanced options expose only field-safe print helper controls", () => {
  ...
  assert.match(indexHtml, /id="balePrintModalLocalAgentPrintButton"[^>]*>打印当前标签<\/button>/);
  ...
  assert.match(advancedSection, /下载 \/ 打开 Windows 打印助手/);
  assert.match(advancedSection, /下载后双击运行，保持黑色窗口不要关闭，然后点击检测打印机与代理/);
