...
test("high-risk scan pages call the global resolver with explicit context", () => {
  ...
  assert.match(appJs, /resolveBarcodeForContext\(scanCode,\s*"clerk_putaway",\s*\["STORE_DELIVERY_PACKAGE"\]\)/);
...
test("PDA keeps STORE_ITEM shelving guarded while SDP scans open assigned package tasks", () => {
  assert.match(appJs, /resolveBarcodeForContext\(payload\.token_no,\s*"store_pda",\s*\["STORE_ITEM"\]\)/);
  assert.match(appJs, /function handleStorePdaSdoPackageScan/);
  assert.match(appJs, /\/store-delivery-packages\/assigned/);
  assert.match(appJs, /function getStorePdaScanGuidance/);
  assert.match(appJs, /请扫描 STORE_ITEM 商品码。/);
  assert.match(appJs, /扫描 SDP 实体包后会校验是否分配给你。/);
  assert.match(appJs, /这是整张送货单，请由店长在门店收货页面处理。/);
  assert.match(appJs, /这是 SDB \/ LPK 来源包，不能直接上架。/);
...
test("pre-QA POS and PDA wrong barcode guidance remains short and actionable", () => {
  ...
  assert.match(pdaGuidance, /这是整张送货单，请由店长在门店收货页面处理。/);
  assert.match(pdaGuidance, /这是 SDB \/ LPK 来源包，不能直接上架。/);
...
test("bale print modal moves technical print controls into collapsed advanced options", () => {
  ...
  assert.match(advancedHtml, /id="balePrintModalCheckLocalAgentButton"[\s\S]*?检测打印助手和打印机/);
  assert.match(advancedHtml, /id="balePrintModalCheckLocalPrintersButton"[\s\S]*?重新检测/);
  assert.match(advancedHtml, /id="balePrintModalLocalAgentPrintButton"[\s\S]*?打印当前标签/);
  ...
  assert.match(advancedHtml, /下载后双击运行，保持黑色窗口不要关闭，然后点击检测打印助手和打印机/);
...
test("bale print modal includes local print agent status and controls", () => {
  ...
  assert.match(indexHtml, /id="balePrintModalCheckLocalAgentButton"[\s\S]*检测打印助手和打印机/);
...
test("field advanced print controls stay limited to safe operator actions", () => {
  ...
  assert.ok(advancedOptions, "advanced print options should exist");
  const advancedHtml = advancedOptions[0];
  assert.match(advancedHtml, /检测打印助手和打印机/);
