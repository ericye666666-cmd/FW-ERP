...
test("POS hotfix keeps top header compact and receipt non-blocking", () => {
  ...
  assert.match(mainBodyRule, /grid-template-columns:\s*minmax\(0,\s*1fr\)\s+clamp\(520px,\s*34vw,\s*580px\)/);
...
test("POS 1366x768 cashier layout keeps scan cart checkout in the first viewport", () => {
  ...
  assert.match(compactDesktopMedia, /body\.cashier-terminal-mode \.cashier-terminal-body\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s+500px/);
